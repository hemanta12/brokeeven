...
- **Settle-mode choice** (`SettleModeChoice.tsx`, Balances tab, added 2026-09-06): a **radio group**, not tabs and not a switch strip. This is a setting that changes what `Settle Up` writes against, so the shape has to describe each option before it is picked — three tab-shaped variants were built and rejected for exactly that reason. It renders only when simplifying actually reduces the payment count, so a group it cannot help never sees it.
- **Close-out review screen** (`CloseGroupModal.tsx`, added 2026-09-06): a destructive-consequence review, so the threshold input and the list of balances it writes off sit on screen together and the list recomputes per keystroke — the number and its consequence are never one tap apart. Confirm stays disabled while real payments remain; a balanced ledger renders its figure in `--text-zero`, the third and last deliberate site for that token. No new colour, no warning-red panel: the disabled action and the visible write-off list carry the weight.
- **Closed (read-only) group**: the closed state removes actions rather than disabling a screenful of them, and says once, in plain copy, that the group is closed and how to reopen it. A disabled control the viewer cannot explain is worse than an absent one (same reasoning as the ownership case in `ExpenseDetail`).
- **Payment handle row** (`SettleUpModal`, added 2026-09-06): sits above the Note field and only when the payee has a handle — an absent handle shows nothing, not an empty-state prompt to go set one. The handle renders in Geist Mono (it is an identifier to be transcribed, like an amount), truncated rather than wrapped, with a `secondary`/`sm` Copy button. Confirmation follows the established pattern: `vibrateConfirm()` plus the button label swapping to "✓ Copied" for two seconds, so the confirmation is visible and not only felt.
