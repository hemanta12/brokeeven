...
// writes pass through: a form always sets a Content-Type, and a bodyless
// cross-site POST cannot carry the fields any route needs.
export const requireJsonWrites: RequestHandler = (request, response, next) => {
  ...
  // request, which would reject writes carrying parameters in the URL (claim,
  // logout, remove person).
